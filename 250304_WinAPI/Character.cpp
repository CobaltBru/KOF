	...
	if (currentState == STATE::PROCESS) //기술중
	{
		if (currentSkill != -1)
		{
			if (framecnt == skillSet[currentSkill].attackFrame)
			{
				...
				FPOINT tempPos = { pos.x,pos.y+50.f };
				FPOINT endPoint = { tempPos.x + (float)((player == 1 ? 1 : -1) * skillSet[currentSkill].reach + (float)(player == 1 ? 0 : 50)), tempPos.y };
				if (CollisionManager::GetInstance()->LineTraceByObject(hit, OBJ_CHARACTER, tempPos, endPoint, this, true))
				{
